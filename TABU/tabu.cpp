...
long tabu(Graph& g, int* c, int k, unsigned long long maxChecks, int staticTenure, int verbose, int freq, int inc, int** neighbors)
{
	...
	while (numConfChecks < maxChecks) {
				...
				if (color != c[node]) {
					...
					if (newValue <= bestValue && color != c[node]) {
						...
						if (tabuStatus[node][color] < totalIterations || (newValue < bestSolutionValue)) {
							...
							std::uniform_int_distribution<int> r1(0, numBest + 1);
							if ( r1(mt2) == 0) {
		...
		if (bestNode == -1) {
			std::uniform_int_distribution<int> r_n(0, g.n - 1);
			std::uniform_int_distribution<int> r_k(0, k - 1);
			bestNode = r_n(mt2);
			numConfChecks++;
			while ((bestColor = r_k(mt2) + 1) != c[bestNode]) numConfChecks++;
			numConfChecks += 2;
		...
		std::uniform_int_distribution<int> r_t(0, tTenure - 1);
		if (randomTenure == 1) tTenure = r_t(mt2) + 1;
		...
		if (staticTenure == 0) {
			...
			std::uniform_int_distribution<int> r_k(0, 9);
			tabuTenure = (int)(0.6 * nc) + r_k(mt2);
		}
...
int Abstract_New_Energy(Graph& g, int* c, int k, unsigned long long maxChecks) {
...
long Abstract_Tabu(Graph& g, int* c, int k, unsigned long long maxChecks, int staticTenure, int verbose, int freq, int inc, int** neighbors)
{
	int** nodesByColor; // Arrays of nodes for each color
	int* nbcPosition;   // Position of each node in the above array
	int** conflicts;   // Number of conflicts for each color and node
